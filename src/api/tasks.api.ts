...
export const getTasks = ({projectId, labelsQuery, statusQuery, priorityQuery}: IGetTasks) => api.get<ITask[]>(buildEndpoint({url: endpoints.getTasks(projectId), queries: [{name: 'status', value: statusQuery}, {name:'labels', value: labelsQuery}, {name:'priority', value: priorityQuery}]}));
...
export const editTask = ({projectId, taskId, body}:{projectId: string, taskId: string, body:ITaskForm}) => api.put<ITask>(endpoints.editTask({projectId, taskId}), body)